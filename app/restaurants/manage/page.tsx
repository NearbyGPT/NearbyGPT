...
import { getRestaurants, createRestaurant, updateRestaurant, deleteRestaurant } from '@/lib/restaurantApi'
...
export default function ManageRestaurantPage() {
  ...
  const handleSubmit = async (data: RestaurantFormData | Partial<RestaurantFormData>) => {
    ...
    try {
      if (mode === 'edit' && selectedRestaurantId) {
        ...
        setRestaurants((prev) => prev.map((r) => (r.id === selectedRestaurantId ? updated : r)))
      ...
      toast.error(mode === 'edit' ? 'Failed to update restaurant' : 'Failed to create restaurant')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-5xl">
      <div className="mb-8 flex items-start gap-x-2">
        <button aria-label="close" className="p-1 rounded-md mt-[0.5rem] bg-white" onClick={() => router.back()}>
          <ChevronLeft size={20} className="text-black" />
        </button>
        <div>
          <h1 className="text-3xl font-bold mb-2">Manage Restaurants</h1>
          <p className="text-muted-foreground">Create a new restaurant or edit an existing one</p>
        </div>
      </div>
      ...
      <Card className="mb-8">
        <CardHeader>
          ...
          <CardDescription>Choose to create a new restaurant or edit an existing one</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 space-y-2">
              ...
              >
                <SelectTrigger id="restaurant-select">
                  <SelectValue
                    placeholder={isFetchingRestaurants ? 'Loading restaurants...' : 'Select a restaurant to edit'}
                  />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-end gap-2">
              <Button type="button" variant="outline" onClick={handleCreateNew} disabled={mode === 'create'}>
